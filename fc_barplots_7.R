...
x35SCO.wt <- read.table(file = "tablas/contraste_35SCO_wt_7.txt", sep = "\t", header = TRUE)
head(x35SCO.wt)
co10.wt <- read.table(file = "tablas/contraste_co10_wt_7.txt", sep = "\t", header = TRUE)
head(co10.wt)
...
## Plot the table
png(filename = "barplot_meristem_determinacy_and_flower_development_7d.png", width = 10, height = 4, units = "in", res = 300)
ggplot(fc.barplot.data, aes(x = gene, y=fc.values, fill=comparison)) +
  ...
  labs(title = "Meristem determinacy and flower development 7 DAG") +
  xlab("") + 
...
## Plot the table
png(filename = "barplot_photoprotection_7d.png", width = 10, height = 4, units = "in", res = 300)
ggplot(fc.barplot.data, aes(x = gene, y=fc.values, fill=comparison)) +
  scale_x_discrete(limits=gene)+
  geom_bar(stat = 'identity', position = "dodge") +
  scale_fill_manual(values=c("darkolivegreen3","darkorange2")) +
  labs(title= "Photoprotection 7 DAG") +
  xlab("") + 
  ylab("Log2(fold-change)") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
...
## Plot the table
png(filename = "barplot_oxidative_stress_7d.png", width = 10, height = 4, units = "in", res = 300)
ggplot(fc.barplot.data, aes(x = gene, y=fc.values, fill=comparison)) +
  scale_x_discrete(limits=gene)+
  geom_bar(stat = 'identity', position = "dodge") +
  scale_fill_manual(values=c("darkolivegreen3","darkorange2")) +
  labs(title="Oxidative stress 7 DAG") +
  xlab("") + 
  ylab("Log2(fold-change)") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
...
## Plot the table
png(filename = "barplot_pigment_biosynthesis_7d.png", width = 10, height = 4, units = "in", res = 300)
ggplot(fc.barplot.data, aes(x = gene, y=fc.values, fill=comparison)) +
  scale_x_discrete(limits=gene)+
  geom_bar(stat = 'identity', position = "dodge") +
  scale_fill_manual(values=c("darkolivegreen3","darkorange2")) +
  labs(title="Pigment biosynthesis 7 DAG") +
  xlab("") + 
  ylab("Log2(fold-change)") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
