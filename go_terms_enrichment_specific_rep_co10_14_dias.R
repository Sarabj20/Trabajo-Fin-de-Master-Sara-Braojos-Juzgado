...
target.genes <- read.table(file="../../tablas/specific_rep_co10_14_dias.txt")
...
#write.table(go.result.table, file = "go_terms_table_specific_rep_co10_14_dias.txt", sep = "\t", row.names = F, quote = F)
...
png(filename = 'barplot_specific_rep_co10_14days.png', width =10, height = 15,units = 'in',res = 300)
barplot(enrich.go,drop=TRUE,showCategory = 40)
...
key.terms <- c("GO:0009908")

keep <- go.result.table$`GO ID`[!go.result.table$`GO ID` %in% key.terms]
specific.go <- dropGO(enrich.go, term = keep )

png(filename = "barplot_rep_co10_specific_14_selected.png", width = 10, height = 3, units = "in", res = 300)
barplot(specific.go,drop=TRUE,showCategory = 40, title = "Specific genes 14 DAG")
dev.off()
...
png(filename = 'treemap_specific_rep_co10_14days.png', width =20, height = 10,units = 'in',res = 300)
treemapPlot(reducedTerms)
