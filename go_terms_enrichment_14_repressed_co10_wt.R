...
target.genes <- read.table(file="../tablas/repressed_genes_co10_wt_14d.txt")
...
#write.table(go.result.table, file = "go_terms_table_repressed_co10_wt.txt", sep = "\t", row.names = F, quote = F)
...
png(filename = 'barplot_repressed_co10_wt_14days.png', width =8, height = 5,units = 'in',res = 300)
...
key.terms <- c("GO:0009908", "GO:0010022")

keep <- go.result.table$`GO ID`[!go.result.table$`GO ID` %in% key.terms]
specific.go <- dropGO(enrich.go, term = keep )

png(filename = "cntplot_co10_flower.png", width = 5, height = 5, units = "in", res = 300)
...
png(filename = 'treemap_repressed_co10_wt_14days.png', width =20, height = 10,units = 'in',res = 300)
treemapPlot(reducedTerms)
