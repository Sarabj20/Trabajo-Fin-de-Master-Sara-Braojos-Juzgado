...
target.genes <- read.table(file="../../tablas/specific_rep_co10_7_dias.txt")
...
#write.table(go.result.table, file = "go_terms_table_specific_rep_co10_7_dias.txt", sep = "\t", row.names = F, quote = F)
...
png(filename = 'barplot_specific_rep_co10_7days.png', width =10, height = 15,units = 'in',res = 300)
barplot(enrich.go,drop=TRUE,showCategory = 40)
...
key.terms <- c("GO:0009753", "GO:0070542", "GO:0001666", "GO:0048639",
               "GO:0046620")

keep <- go.result.table$`GO ID`[!go.result.table$`GO ID` %in% key.terms]
specific.go <- dropGO(enrich.go, term = keep )

png(filename = "barplot_rep_co10_specific_7_selected.png", width = 10, height = 3, units = "in", res = 300)
...
png(filename = 'treemap_specific_rep_co10_7days.png', width =20, height = 10,units = 'in',res = 300)
treemapPlot(reducedTerms)
