...
target.genes <- read.table(file="../../tablas/specific_act_35sco_14_dias.txt")
...
#write.table(go.result.table, file = "go_terms_table_specific_act_35sco_14_dias.txt", sep = "\t", row.names = F, quote = F)
...
png(filename = 'barplot_specific_act_35sco_14days.png', width =10, height = 15,units = 'in',res = 300)
barplot(enrich.go,drop=TRUE,showCategory = 40)
...
key.terms <- c("GO:0042742", "GO:0050832", "GO:0002213", "GO:0002229",
               "GO:0009615", "GO:0048437", "GO:0048438")


keep <- go.result.table$`GO ID`[!go.result.table$`GO ID` %in% key.terms]
specific.go <- dropGO(enrich.go, term = keep )

png(filename = "barplot_activ_35sco_specific_14_selected.png", width = 10, height = 3, units = "in", res = 300)
barplot(specific.go,drop=TRUE,showCategory = 20, title = "Specific genes 14 DAG") 
dev.off()
...
png(filename = 'treemap_specific_act_35sco_14days.png', width =20, height = 10,units = 'in',res = 300)
treemapPlot(reducedTerms)
