...
target.genes <- read.table(file="../../tablas/rep_co10_7_14_dias.txt")
...
#write.table(go.result.table, file = "go_terms_table_rep_co10_7_14_dias.txt", sep = "\t", row.names = F, quote = F)
...
png(filename = 'barplot_rep_co10_7_14days.png', width =10, height = 15,units = 'in',res = 300)
barplot(enrich.go,drop=TRUE,showCategory = 40)
...
# GO:2000030 -> response to red or red far light
key.terms <- c("GO:0010022", "GO:0007623", "GO:0009908", "GO:0010167", "GO:2000030")

keep <- go.result.table$`GO ID`[!go.result.table$`GO ID` %in% key.terms]
specific.go <- dropGO(enrich.go, term = keep )

png(filename = "barplot_rep_co10_common_7_14_selected.png", width = 10, height = 3, units = "in", res = 300)
barplot(specific.go,drop=TRUE,showCategory = 40, title = "Common genes 7 and 14 DAG")
dev.off()
...
png(filename = 'treemap_rep_co10_7_14days.png', width =20, height = 10,units = 'in',res = 300)
treemapPlot(reducedTerms)
